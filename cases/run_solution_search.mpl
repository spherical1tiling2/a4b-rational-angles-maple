...
tol := 10^(-28):
if FileTools:-Exists("solution_range.mpl") then
  read "solution_range.mpl":
end if:
...
foundExact := []: foundUnique := []: degreeExcluded := []:
degreeFile := fopen("degree_bound_exclusions.csv",WRITE):
fprintf(degreeFile,"case,f,angles_pi,minimum_f_from_vertex_degrees\n"):
fclose(degreeFile):
checkpointFile := "solution_checkpoint.txt":
...
if FileTools:-Exists("solution_candidates.csv") then
  csvText := FileTools:-Text:-ReadFile("solution_candidates.csv"):
  if type(csvText,string) then
    if StringTools:-Trim(csvText)<>"" then
      lf := fopen("solution_candidates.csv",APPEND):
    else
      lf := fopen("solution_candidates.csv",WRITE):
...
Progress := proc(msg)
  local lp;
  lp := fopen("solution_progress.log",APPEND):
  fprintf(lp,"%s\n",msg):
  fclose(lp):
end proc:
Progress("START solution search"):
...
`a4b_solver/AcceptAffineCandidate` := proc(id,av,branch,lf)
  local qv,fv,res,keye,keyu,cf,minF,v;
  global foundExact, foundUnique, degreeExcluded;
  ...
  keyu := cat(fv,"|",av[1..5]):
  # Euler's formula gives f=12+2*sum((k-3)*v_k,k>=4).
  # Each required vertex type occurs at least once. In particular, a
  # required vertex of degree >=4 implies f>=14.
  minF := 12+2*add(add(v)-3,v in caseByID[id][2]):
  if fv<minF then
    if not member(keye,degreeExcluded) then
      degreeExcluded := [op(degreeExcluded),keye]:
      cf := fopen("degree_bound_exclusions.csv",APPEND):
      fprintf(cf,"%s,%a,\"%a\",%a\n",id,fv,av[1..5],minF): fclose(cf):
    end if:
    return NULL
  end if:
...
treeLog := fopen("solution_factors.csv",WRITE):
...
for idx from first_case to last_case do
  ...
  # Every listed vertex combination is computed, including OR branches.
  processed:=processed+1: id:=row[1]:
  primaryID:=id: orPosition:=StringTools:-Search("-OR",id):
  if orPosition>0 then primaryID:=substring(id,1..orPosition-1) end if:
  Progress(sprintf("CASE_BEGIN %s raw_index=%d record=%d",id,idx,processed)):
  try
    X:='X':Y:='Y':zeta:='zeta':
    ...
    for idxf from 1 to nops(fac) do
      ...
      for idxg from 1 to nops(fieldfac) do
        ...
        if nops(active)=2 then
          ...
          if reduceBeforeNorm and Q["N"]<>1 and member(primaryID,normCaseIDs) and baseRed["status"]="reduced" then
          ...
          if Q["N"]<>1 and member(primaryID,normCaseIDs) then
            try
...
cmp:=fopen("solution_comparison.md",WRITE):
fprintf(cmp,"# Independent solution comparison\n\n"):
fprintf(cmp,"- Processed three-vertex cases: %d\n",processed):
fprintf(cmp,"- Cases covered through raw index %d: %d\n",last_case,coveredPrimary):
